// 共享工具函数：时间格式化、用户模型转响应实体

import type { UserInfo, UserState, Sex } from "../entity/user";
import type { UserGetPayload } from "../generated/prisma/models/User";

// 格式化为 "YYYY-MM-DD HH:mm:ss"，与 Go time.DateTime 一致
export function formatDateTime(d: Date): string {
  const pad = (n: number) => String(n).padStart(2, "0");
  return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;
}

// 公开/鉴权路径查询用户时的列白名单 (单一数据源)：
// 相比 omit: { password: true } 只排除单列, 精确 select 可避免把 password/updatedAt
// 等无关列拉进内存并参与传输 (对齐 Go 端 UserColumnsNoPassword 的语义)
export const userPublicSelect = {
  id: true,
  typeId: true,
  account: true,
  mobile: true,
  email: true,
  nickname: true,
  avatarUrl: true,
  sex: true,
  states: true,
  createdAt: true,
} as const;

// 响应映射所需的字段子集: 直接从 userPublicSelect 推导 (单一数据源) —
// schema 或 select 变更时类型自动同步, 消除"手写 Pick 类型 + select 对象"双份维护的漂移风险
type UserInfoSource = UserGetPayload<{ select: typeof userPublicSelect }>;

// 将 Prisma User 转为 UserInfo（不含 password 等敏感字段）
// 复合词 JSON key 使用 snake_case 以与 Go API 保持一致
export function toUserInfo(user: UserInfoSource): UserInfo {
  return {
    uid: user.id,
    type_id: user.typeId,
    account: user.account,
    mobile: user.mobile,
    email: user.email,
    nickname: user.nickname,
    avatar_url: user.avatarUrl || "",
    sex: user.sex as Sex,
    states: user.states as UserState,
    created_at: formatDateTime(user.createdAt),
  };
}
