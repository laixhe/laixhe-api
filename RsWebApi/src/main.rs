...
async fn main() {
    ...
    // --gen-docs: 仅根据注解重新生成 swagger 文档到 docs/ 目录 (对齐 Go 端 `make swag`), 不启动服务
    if std::env::args().any(|a| a == "--gen-docs") {
        let json = docs::ApiDoc::openapi().to_json().expect("openapi to_json failed");
        let yaml = docs::ApiDoc::openapi().to_yaml().expect("openapi to_yaml failed");
        std::fs::write("docs/swagger.json", json).expect("write docs/swagger.json failed");
        std::fs::write("docs/swagger.yaml", yaml).expect("write docs/swagger.yaml failed");
        println!("swagger docs regenerated -> docs/swagger.json / docs/swagger.yaml");
        return;
    }
...
fn parse_args() -> String {
    ...
    // (用 if let 解构 Option, 避免 is_some() + unwrap() 反模式)
    if let Some(env_name) = env_name {
        if config_file == "./config.yaml" {
            let env_file = format!("./config.{env_name}.yaml");
