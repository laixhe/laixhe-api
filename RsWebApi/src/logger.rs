...
//! let step = Timer::new();
//! // ... 业务操作 ...
//! // 注意: 耗时日志统一用 debug 级 (逐请求输出, 避免 info 级在高 QPS 下产生大量日志开销)
//! log_elapsed!(step, elapsed_ms, debug, email = %req.email, "检查通过");
...
/// // 与全项目实践保持一致, 耗时日志统一使用 debug 级
/// log_elapsed!(step, elapsed_ms, debug, email = %req.email, "邮箱检查通过");
/// log_elapsed!(start, total_ms, debug, email = %req.email, uid, "注册完成");
