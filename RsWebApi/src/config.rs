...
pub struct LogConfig {
    ...
    /// 日志文件路径
    pub path: String,
    /// 日志级别 debug info warn error (缺省 info, 见 default_log_level)
...
/// 接口限流配置
///
/// 字段级缺省即为"整段缺失"时的默认值 (enable=true, max=1000, window=60)。
/// 注意这与 Go 原版 mapstructure 的零值语义 (enable 缺省 false) 刻意不同:
/// Go 版"写了 limit 段但漏写 enable"时限流会被静默关闭, 是本版修掉的配置陷阱。
/// 显式写 `enable: false` 仍可关闭限流。
...
/// enable 字段缺省值 (与 Config::load 中整段缺失时的默认一致)
...
impl Config {
    ...
    pub fn load(config_file: &str) -> Result<Config, String> {
        ...
        let config = Config {
            ...
            // limit 整段缺失时补默认 {enable, 1000, 60} (与字段级缺省默认保持一致)
    ...
    pub fn check(&self) -> Result<(), String> {
        ...
        // 启用限流时 max 必须为正数 (显式写 max: 0 属于配置错误, 启动时直接暴露)
        if self.limit.enable && self.limit.max == 0 {
            return Err("limit max must be > 0 when rate limit enabled".to_string());
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn expand_env_substitutes_undefined_variables_as_empty() {
        // 未设置的环境变量展开为空串 (与 Go os.ExpandEnv 行为一致)
        assert_eq!(expand_env("${NOT_SET_XYZ}"), "");
        assert_eq!(expand_env("$NOT_SET_XYZ"), "");
        // 无变量时原样返回
        assert_eq!(expand_env("hello world"), "hello world");
        // 变量前后普通字符保留
        assert_eq!(expand_env("a${NOT_SET_XYZ}b"), "ab");
        // 中文等多字节字符不被字节下标逻辑截断
        assert_eq!(expand_env("你好"), "你好");
        assert_eq!(expand_env("你好$NOT_SET_XYZ"), "你好");
        // 数字开头的 $ 不是变量名 ($1 按普通字符处理)
        assert_eq!(expand_env("$1abc"), "$1abc");
    }

    #[test]
    fn expand_env_substitutes_set_variables() {
        // 设置环境变量后应正确替换 (测试进程内生效, 不影响其它用例)
        // 注意: 2024 edition 下 set_var/remove_var 为 unsafe; 测试为单线程顺序执行, 无数据竞争
        unsafe {
            std::env::set_var("TEST_EXPAND_1", "value1");
        }
        assert_eq!(expand_env("${TEST_EXPAND_1}"), "value1");
        assert_eq!(expand_env("$TEST_EXPAND_1"), "value1");
        assert_eq!(expand_env("x${TEST_EXPAND_1}y"), "xvalue1y");
        unsafe {
            std::env::remove_var("TEST_EXPAND_1");
        }
    }
}
