...
use crate::app::entity::user::{User, UserInfoRequest, UserListRequest, UserListResponse, UserUpdateRequest};
...
pub async fn update(state: &AppState, req: &UserUpdateRequest) -> Result<User, ApiError> {
    let start = Timer::new();
    tracing::debug!(uid = req.uid, "开始更新用户");
...
#[cfg(test)]
mod tests {
    use super::page_offset_calculation;

    /// 分页计算边界: 正常分页 limit/offset 正确, 非法参数兜底为 1 (与 Go 端 normalizePagination 语义对齐)
    #[test]
    fn page_offset_calculation_boundaries() {
        // 正常分页: limit = page_size, offset = (page - 1) * page_size
        assert_eq!(page_offset_calculation(1, 12), (12, 0));
        assert_eq!(page_offset_calculation(2, 12), (12, 12));
        assert_eq!(page_offset_calculation(3, 5), (5, 10));
        // page <= 0 兜底为 1 (控制器已钳制, service 层双保险)
        assert_eq!(page_offset_calculation(0, 12), (12, 0));
        assert_eq!(page_offset_calculation(-5, 12), (12, 0));
        // page_size <= 0 兜底为 1
        assert_eq!(page_offset_calculation(3, 0), (1, 2));
        assert_eq!(page_offset_calculation(3, -5), (1, 2));
        // 边界: page_size = 1 时 offset 按 page 递增
        assert_eq!(page_offset_calculation(2, 1), (1, 1));
    }
}
