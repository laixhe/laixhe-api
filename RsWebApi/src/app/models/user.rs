...
pub async fn find_by_id(db: &DatabaseConnection, uid: u32) -> Result<Option<Model>, DbErr> {
...
pub async fn find_id_by_email(db: &DatabaseConnection, email: &str) -> Result<Option<u32>, DbErr> {
    Entity::find()
        .select_only()
        .column(Column::Id)
...
pub async fn create_user(db: &DatabaseConnection, user: ActiveModel) -> Result<u32, DbErr> {
    let txn = db.begin().await?;
    let result = Entity::insert(user).exec(&txn).await?;
    let uid = result.last_insert_id as u32;
