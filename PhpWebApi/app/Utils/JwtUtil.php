...
class JwtUtil
{
    ...
    public function createToken(int $uid, array $claims=[]): string
    {
        ...
        try {
            ...
            if (empty($expiresAt)) {
                throw new RuntimeException('过期时间生成失败');
            }
            ...
            if (!empty($claims)) {
                ...
                if (count($claims_keys) !== count(array_filter($claims_keys, 'is_string'))){
                    throw new RuntimeException('claims 参数必须为关联数组');
                }
            }
            ...
            throw new RuntimeException('创建 token '.$e->getMessage(), ResultCode::Unknown->value);
        }
    }
