...
{
    return response()->json(new Result(ResultCode::Success, '', $data));
}

function response_error(ResultCode $code, string $msg = '', array $data = []): JsonResponse
{
    return response()->json(new Result($code, $msg ?: $code->text(), $data));
}

function response_exception(int $code, string $msg = '', array $data = []): JsonResponse
{
    $result = ResultCode::intToEnum($code);
    return response()->json(new Result($result, $msg ?: $result->text(), $data));
}
