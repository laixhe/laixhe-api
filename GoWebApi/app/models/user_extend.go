...
// UserExtendTable 用户扩展表名
const UserExtendTable = "user_extend"

// UserExtend 用户扩展
type UserExtend struct {
	ID       int `gorm:"column:id;type:int;autoIncrement;primaryKey"`
	Uid      int `gorm:"column:uid;type:int;not null;uniqueIndex;comment:用户UID"`
	Birthday int `gorm:"column:birthday;type:int;not null;default:0;comment:生日(年月日)"`
