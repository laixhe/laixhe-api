...
func (u *User) FirstUname(uname string) (User, error) {
	...
	err := gormx.Where("uname", uname).First(&user).Error
	return user, err
}
...
func (u *User) List(size, page int) ([]User, int64, error) {
	...
	err := gormx.Model(u).Count(&total).Error
	if err != nil || total == 0 {
		return nil, 0, err
	}
...
func (u *User) Update(user *User) error {
	...
	return gormx.Model(u).Select([]string{"uname", "login_at"}).Where("id", user.Uid).Updates(user).Error
}
