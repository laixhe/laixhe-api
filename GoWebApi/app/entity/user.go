...
type User struct {
	...
	// UserType:
	// * 1 - 普通用户
	TypeId    models.UserType `json:"type_id" validate:"required" swaggertype:"integer" enums:"1"` // 类型
	...
	// * 1 - 男
	// * 2 - 女
	Sex models.UserSex `json:"sex" validate:"required" swaggertype:"integer" enums:"0,1,2"` // 性别
	// UserState:
	// * 0 - 禁用
	// * 1 - 正常
	States    models.UserState `json:"states" validate:"required" swaggertype:"integer" enums:"0,1"` // 状态
...
//
// 注意: 不要加 validate:"required" (缺省时归一化为 1/12, 与 Rust/TS/PHP 端 required:false 对齐)
type UserListRequest struct {
	Page     int `query:"page" json:"page"`           // 分页-当前页(默认 1)
	PageSize int `query:"page_size" json:"page_size"` // 分页-每页数量(默认 12)
}
