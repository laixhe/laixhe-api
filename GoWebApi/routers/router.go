...
func (r *Router) init() *Router {
	...
	groupApi := r.server.Fiber().App().Group("api")
	{
		groupApiV1 := groupApi.Group("v1")
		{
			...
			groupApiV1.Get("swagger", func(ctx fiber.Ctx) error {
				ctx.Set(fiber.HeaderContentType, "text/html; charset=utf-8")
				ctx.Set(fiber.HeaderCacheControl, "public, max-age=300")
				return ctx.SendString(docs.SwaggerUiHtml)
			})
	...
	// 404 统一 JSON 兜底 (与 Rust/TS/PHP 端对齐): 未匹配路由返回 {"code":404,"message":"Not Found"}
	// 注意: Use 注册在业务路由之后, 已匹配的请求在路由 handler 处即返回, 走到这里的都是未匹配路由
	r.server.Fiber().App().Use(func(ctx fiber.Ctx) error {
